...
public class DataManager implements DataManagerConst {
    ...
    private File mLogDir, mDataDir, mImageDir, mMusicDir;
...
//        mPushAgent = PushAgent.getInstance(context);
     ...
     * @return
     */
    public String getMuiscPath() {
        return mMusicDir.getPath() + "/muyin_music";
    }
    ...
    public boolean isShopUser() {
        if (!StringUtil.isBlank(readTempData(ZnzConstants.ACCOUNT_TYPE))) {
            if (readTempData(ZnzConstants.ACCOUNT_TYPE).equals("1")) {
                return true;
            } else {
                return false;
            }
    ...
    public void logout(Activity activity, Class<?> cls) {
        ...
        clearData();
        ZnzRetrofitUtil.getInstance().reset();
     ...
     * @param resId
     */
    public void setTextDrawableTop(TextView text, @DrawableRes int resId) {
        Drawable drawable = context.getResources().getDrawable(resId);
        drawable.setBounds(0, 0, drawable.getMinimumWidth(), drawable.getMinimumHeight());
                ...
                .subscribe(granted -> {
                    if (granted) {
                        ...
                        new AlertDialog.Builder(activity)
                                .setTitle("权限申请")
                                .setMessage("该操作需要拨打电话权限，请在设置中打开该应用的权限")
                                .setCancelable(false)
                                .setNegativeButton("取消", null)
    ...
    public void openPhotoSelectSingleWithoutCamera(Activity activity, IPhotoSelectCallback iPhotoSelectCallback, boolean needCrop) {
        GalleryConfig galleryConfig = new GalleryConfig.Builder()
                .imageLoader(new GlideImageLoader())    // ImageLoader 加载框架（必填）
                .iHandlerCallBack(iPhotoSelectCallback)     // 监听接口（必填）
                .multiSelect(false)                      // 是否多选   默认：false
...
//        }, String... tags);
    ...
    public Bitmap createViewBitmap(View addViewContent) {
